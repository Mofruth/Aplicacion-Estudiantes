...
## 1. Autenticación unificada por correo y contraseña
- Eliminada la selección manual de rol en el formulario de inicio de sesión.
- El login ahora valida únicamente por correo y contraseña.
- El sistema detecta automáticamente el tipo de usuario (administrador, profesor, estudiante, padre) tras autenticar.

## 2. Refactorización de la autenticación
- Eliminada la clase `AutenticacionController`.
- Implementada la función `autenticar_usuario` en `Controllers/autenticacion.py`, que busca el usuario en todas las tablas de roles y valida la contraseña.
- Eliminadas todas las referencias a `AutenticacionController` en el proyecto.

## 3. Método `obtener_por_correo` en modelos
- Agregado el método estático `obtener_por_correo` en los modelos `admin.py`, `profesor.py`, `estudiante.py` y `padre.py` para facilitar la autenticación centralizada.

## 4. Actualización de la vista de login
- El formulario de login (`Views/auth/login.html`) ahora solo solicita correo y contraseña.
- Eliminado cualquier campo o lógica relacionada con la selección de rol.

## 5. Redirección automática según rol
- Tras el login, el sistema redirige automáticamente al dashboard correspondiente según el tipo de usuario autenticado.

## 6. Corrección de endpoints en plantillas
- Corregidos enlaces en las plantillas base (`base_form.html` y otras) para usar los endpoints correctos (`url_for('home')`, `url_for('login')`, `url_for('logout')`), eliminando prefijos de blueprints inexistentes.

## 7. Limpieza y organización
- Eliminadas rutas y referencias duplicadas o innecesarias en `main.py` y otros archivos.
- Revisados y ajustados los imports y el uso de sesión para mantener consistencia en todo el proyecto.
